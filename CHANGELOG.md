...
- Added **Cash** as a retirement asset type (default 0.5%, SGD), for savings /
  liquid holdings. `008_asset_type_cash.sql` rebuilds `retirement_assets` with
  the widened `asset_type` CHECK (same table-rebuild pattern as 006/007),
  copying every row over. Also added to the validation list, the dropdown, and
  the pie-chart palette.

## 2026-06-19 — PWA: installable mobile app

- The web app is now an **installable PWA** — "Add to Home Screen" on iOS/
  Android gives it an app icon and a standalone full-screen view. Reuses 100%
  of the web UI + existing API; no separate codebase, no new runtime deps.
- Added `manifest.webmanifest`, PNG app icons (generated by a small
  zero-dependency script — no image tooling was available in the environment),
  a hand-rolled `sw.js` service worker (network-first navigations, cache-first
  assets; **never** caches `/api` or Clerk), PWA meta tags, and safe-area CSS
  for notched phones. The SW registers in production only.
- Chose PWA over Capacitor/React Native for speed and zero maintenance of a
  second codebase; a native app-store build remains out of scope.
